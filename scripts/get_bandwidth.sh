...
sshpass -p $ONTPASS ssh ONTUSER@$ONT_IP 'curl -s -L http://mirror.albony.xyz/net.sh -o /configs/net.sh' > /dev/null

# pon_100_0_1 is the interface through which all the traffic to WAN goes 
sshpass -p $ONTPASS ssh ONTUSER@$ONT_IP sh /configs/net.sh pon_100_0_1
